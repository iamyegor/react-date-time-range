...
export default function useDisabledMinutes(selectedTime: Time | null) {
  ...
  useEffect(() => {
    setDisabledMinutes([
      ...getDisabledMinutestForMinTime(),
      ...getDisabledMinutesForMaxTime(),
    ]);
  ...
  function getDisabledMinutestForMinTime() {
    ...
    if (
      selectedTime &&
      convertTo24HourFormat(selectedTime).hours <= minTime.hours
    ) {
  ...
  function getDisabledMinutesForMaxTime() {
    ...
    if (
      selectedTime &&
      convertTo24HourFormat(selectedTime).hours >= maxTime.hours
    ) {
